...
public class AdminMemberContAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request,
		...
		if(admin_id == null){
		   ...
		   // 개행문자를 <BR> 태그로 교체
		   if(bean.getMember_delcont() != null){
			   bean.setMember_delcont
			   (bean.getMember_delcont().replace("\n","<br>"));
		   }
		   // 가입 날짜에 대한 날짜 표시
		   bean.setMember_regdate
		      (bean.getMember_regdate().substring(0,10));
		   // 탈퇴 날짜에 대한 날짜 표시
		   if(bean.getMember_deldate() !=null){
			   bean.setMember_deldate
			   (bean.getMember_deldate().substring(0,10));
		   }
