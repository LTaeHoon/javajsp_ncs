...
public class AdminMemberDAO {
	...
	public List<MemberBean> getMemberList(int page, int limit) {
		...
		try{
		    ...
		    while(rs.next()){ // 레코드 수 만큼 반복
		    	...
		    	bean.setMember_regdate
		    	   (rs.getString("member_regdate"));
		    	bean.setMember_state
		    	   (rs.getInt("member_state"));
	...
	public MemberBean getmemCont(String mem_id) {
		...
		try{			
			...
			if(rs.next()){
				...
				bean.setMember_state
			      (rs.getInt("member_state"));
				bean.setMember_delcont
			      (rs.getString("member_delcont"));
				bean.setMember_deldate
			      (rs.getString("member_deldate"));
			}
			rs.close(); pstmt.close(); con.close();
		}catch(Exception e){
			e.printStackTrace();
	...
	public int updateMember(MemberBean bean) {
		...
		try{
			sql="update member set "
		   	   ...
		   	   + " member_nickname=?, member_zip1=?, "
		   	   + " member_zip2=?, member_addr1=?, "
	...
	public List<MemberBean> getMemList3(int page, int limit, String find_name,
		...
		try{					
			...
			+" member_nickname,member_regdate,member_state "
			...
			while(rs.next()){
				...
				mb.setMember_regdate(rs.getString("member_regdate"));	
				mb.setMember_state(rs.getInt("member_state"));
